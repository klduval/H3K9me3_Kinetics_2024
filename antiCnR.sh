...
do
  ...
  bedtools intersect -a $infile -b /work/mglab/kld/TEanns/*0.1*.bed -F 0.50 -wo > $BASEDIR/peaks/ann/$base.TEann.txt
done

for infile in $BASEDIR/peaks/ann2/*.TEann.txt
do
  base=$(basename ${infile} .txt)
...
for infile in $BASEDIR/matrices/abcam*.gz
do
  base=$(basename ${infile} .gz)
  plotHeatmap -m $infile --colorMap Blues --legendLocation none --whatToShow "heatmap and colorbar" --regionsLabel "$base" -o $BASEDIR/figs/"$base"_heatmap.pdf
done

##with the shared/specific peaks
bedtools intersect -a $BASEDIR/peaks/K9abcam_24hpf_final.bed -b $BASEDIR/peaks/K9abcam_4.5hpf_final.bed -v > $BASEDIR/peaks/24hpf_only.bed
bedtools intersect -a $BASEDIR/peaks/K9abcam_4.5hpf_final.bed -b $BASEDIR/peaks/K9abcam_24hpf_final.bed -v > $BASEDIR/peaks/4.5hpf_only.bed
bedtools intersect -a $BASEDIR/peaks/K9abcam_24hpf_final.bed -b $BASEDIR/peaks/K9abcam_4.5hpf_final.bed -wa > $BASEDIR/peaks/24hpf_4.5h_shared.bed

computeMatrix reference-point -S $BASEDIR/bws/K9abcam_4.5hpf_AVG.bw -R $BASEDIR/peaks/4.5hpf_only.bed $BASEDIR/peaks/24hpf_4.5h_shared.bed  $BASEDIR/peaks/24hpf_only.bed --referencePoint center -p max -a 10000 -b 10000 -bs 10 --missingDataAsZero -o $BASEDIR/matrices/4.5hpf_shared_specific_24h_peaks.gz
plotHeatmap -m $BASEDIR/matrices/4.5hpf_shared_specific_24h_peaks.gz --colorMap Blues --legendLocation none --whatToShow "heatmap and colorbar" --outFileSortedRegions $BASEDIR/peaks/4.5h_shared_spec_24h_peaks_sorted_regions.bed -o $BASEDIR/figs/4.5h_shared_spec_24h_peaks_heatmap.pdf
computeMatrix reference-point -S $BASEDIR/bws/K9abcam_24hpf_AVG.bw -R $BASEDIR/peaks/4.5h_shared_spec_24h_peaks_sorted_regions.bed --referencePoint center -p max -a 10000 -b 10000 -bs 10 --missingDataAsZero --sortRegions no -o $BASEDIR/matrices/24hpf_shared_specific_4.5h_peaks.gz
plotHeatmap -m $BASEDIR/matrices/24hpf_shared_specific_4.5h_peaks.gz --colorMap Blues --legendLocation none --whatToShow "heatmap and colorbar" --sortRegions no -o $BASEDIR/figs/24h_shared_spec_4.5h_peaks_heatmap.pdf

computeMatrix scale-regions -S $BASEDIR/bws/K9abcam_4.5hpf_AVG.bw $BASEDIR/bws/K9abcam_24hpf_AVG.bw -R $BASEDIR/peaks/24hpf_4.5h_shared.bed -p max --outFileNameMatrix $BASEDIR/4.5h_24h_shared_peaks_matrix.tab -o $BASEDIR/matrices/5h_24h_shared_peaks_matrix.gz

...
p2hpf <- read_delim("2hpf_K9_final.bed", col_names = bed_col_names) %>%
  mutate(time_point = "2hpf")
p2.5hpf <- read_delim("2.5hpf_K9_final.bed", col_names = bed_col_names) %>%
  mutate(time_point = "2.5hpf")
p3hpf <- read_delim("3hpf_K9_final.bed", col_names = bed_col_names) %>%
  mutate(time_point = "3hpf")
p3.5hpf <- read_delim("3.5hpf_K9_final.bed", col_names = bed_col_names) %>%
  mutate(time_point = "3.5hpf")
p4hpf <- read_delim("4hpf_K9_final.bed", col_names = bed_col_names) %>%
  mutate(time_point = "4hpf")
p4.5hpf <- read_delim("4.5hpf_K9_final.bed", col_names = bed_col_names) %>%
...
ggsave("K9_Kinetics/Fig3/npeaks_bar.pdf", plot = npeaks_bar, width = 2, height = 2.5, units = "in")
...
ggsave("K9_Kinetics/Fig3/peakwidth_viol.pdf", plot = peakwidth_viol, width = 2, height = 2, units = "in")


###now going to read in the peak enrichment matrices so I can calculate peak heights at each time####
h2hpf <- read_delim("2hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
    mutate_if(is.factor, ~as.numeric(as.character(.)))
h2.5hpf <- read_delim("2.5hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
  mutate_if(is.factor, ~as.numeric(as.character(.)))
h3hpf <- read_delim("3hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
  mutate_if(is.factor, ~as.numeric(as.character(.)))
h3.5hpf <- read_delim("3.5hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
  mutate_if(is.factor, ~as.numeric(as.character(.)))
h4hpf <- read_delim("4hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
  mutate_if(is.factor, ~as.numeric(as.character(.)))
h4.5hpf <- read_delim("4.5hpf_peakmatrix.tab", delim = '\t', skip = 2) %>%
  mutate_if(is.factor, ~as.numeric(as.character(.)))
...
ggsave("K9_Kinetics/Fig3/peak_height.pdf", plot = peak_height, width = 2, height = 2.5, units = "in")

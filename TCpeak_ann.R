...
K9_2hpf <- read_delim("2hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
 mutate(timepoint = "2 hpf")
K9_2.5hpf <- read_delim("2.5hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
  mutate(timepoint = "2.5 hpf")
K9_3hpf <- read_delim("3hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
  mutate(timepoint = "3 hpf")
K9_3.5hpf <- read_delim("3.5hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
  mutate(timepoint = "3.5 hpf")
K9_4hpf <- read_delim("4hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
  mutate(timepoint = "4 hpf")
K9_4.5hpf <- read_delim("4.5hpf_K9_.TEann_35_0.1filt_counts.bed", col_names = col_names) %>%
...
ggsave("K9_Kinetics/Fig2/TEprops_4.5hpf_pie.pdf", plot = TE_Props_graph, width = 2, height = 2, units = "in")
...
CN_Props_graph <- ggplot(filter(Class_4.5hpf, prop_type == "CN"), aes(x = Class, y = prop, fill = Class)) +
  ...
  theme(axis.title.x = element_blank())
ggsave("K9_Kinetics/Fig2/CNprops_4.5hpf.pdf", plot = CN_Props_graph, width = 1.5, height = 2, units = "in")
...
Family_20p_cutoff_graph <- ggplot(Family_20p_cutoff, aes(x = Class, y = perc, fill = Class)) +
  ...
  theme(legend.position = "none") +
  theme(axis.title.x = element_blank())
ggsave("K9_Kinetics/Fig2/Family_20p_cutoff.pdf", plot = Family_20p_cutoff_graph, width = 1.5, height = 2, units = "in")
ggsave("K9_Kinetics/Fig2/Family_20p_cutoff_bigger.pdf", plot = Family_20p_cutoff_graph, width = 1.5, height = 3, units = "in")
...
LTRs_20props_pics <- ggplot(LTRs_20props, aes(x = Family, y = prop, fill = Family)) +
  ...
  theme(legend.position = "none") +
  theme(axis.title.x = element_blank())
ggsave("K9_Kinetics/Fig2/LTRs_20props_pics.pdf", plot = LTRs_20props_pics, width = 2.5, height = 2, units = "in")
...
LTRs_20p_cutoff <- ggplot(LTRs_20p_Fams, aes(x = Family, y = perc, fill = Family)) +
  ...
  theme(legend.position = "none") +
  theme(axis.title.x = element_blank())
ggsave("K9_Kinetics/Fig2/LTRs_20p_cutoff.pdf", plot = LTRs_20p_cutoff, width = 2.5, height = 2, units = "in")
...
ggsave("K9_Kinetics/Fig2/cnprop_medlen_scatter.pdf", plot = cnprop_medlen_scatter, width = 5.5, height = 2, units = "in")

####now just compare 3 hpf to 4.5 hpf####
library(ggprism)

col_names <- c("count", "Name")
K9_4.5m3h <- read_delim("4.5m3h_TEann_counts.bed", col_names = col_names) %>%
...
ggsave("K9_Kinetics/Fig4/EGA_comp_class.pdf", plot = EGA_comp_class, width = 2.35, height = 2.75, units = "in")
...
EGA_comp_LTR <- ggplot(EGA_LTRs_trimmed, aes(x = Family, y = EGA_frac)) +
  ...
  add_pvalue(df_pval,
             ...
             label.size = 2.5) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "grey20", linewidth = 0.5)
ggsave("K9_Kinetics/Fig4/EGA_comp_LTR.pdf", plot = EGA_comp_LTR, width = 3.1, height = 2.7, units = "in")
...
####import piRNA data####
piRNA_Table <- read.csv("piRNA_TPM.csv")
piRNAs_w3hK9 <- read_delim("piRNA_3hK9_TEs_counts.bed", col_names = col_names)
...
ggsave("K9_Kinetics/Fig4/LTR_K9_piRNA_plot.pdf", plot = LTR_K9_piRNA_plot, width = 3.2, height = 2.7, units = "in")
...
ggsave("K9_Kinetics/Fig3/upset.pdf", plot = upset, width = 4.5, height = 3, units = "in")



####pre/postEGA peak int w peric####
##imp data##
preEGA_NULL <- read_delim("preEGA_peaks_null_ints.tsv",
                        col_names = FALSE, delim = "/", trim_ws = TRUE) %>% distinct() %>% filter(X1 != 0) %>% select(X1, X6) %>% rename(count = X1, loc = X6)
postEGA_NULL <- read_delim("postEGA_peaks_null_ints.tsv",
                         col_names = FALSE, delim = "/", trim_ws = TRUE) %>% distinct() %>% filter(X1 != 0) %>% select(X1, X6) %>% rename(count = X1, loc = X6)

preEGA_perc <- read_delim("preEGA_peaks.peric.bed")
postEGA_perc <- read_delim("postEGApeaks_comp.peric.bed")
...
ggsave("K9_Kinetics/Fig5/peric_permtest_plot.pdf", plot = peric_permtest_plot, width = 1.75, height = 2.5, units = "in")
